...
const AspectColor = ({colorValue, onRelease}) => {
  ...
  const style = colorValue === "red" ? RED_ASPECT : 
                colorValue === "orange" ? ORANGE_ASPECT : 
                colorValue === "yellow" ? YELLOW_ASPECT : 
                colorValue === "green" ? GREEN_ASPECT : 
                colorValue === "cyan" ? CYAN_ASPECT : 
                colorValue === "blue" ? BLUE_ASPECT : 
                colorValue === "purple" ? PURPLE_ASPECT : INVISIBLE_ASPECT

  return <View>
    <Draggable shouldReverse onDragRelease={(event, gestureState, bounds) => {
...
export function TutorialProfileSetupArea({ saveColorScheme }) {
  ...
  const [moods, setMood] = useReducer((state, action) => {
    const decodeStyleFromColor = (color) => color === "red" ? RED_MOOD_VIEW : 
                                            color === "orange" ? ORANGE_MOOD_VIEW : 
                                            color === "yellow" ? YELLOW_MOOD_VIEW : 
                                            color === "green" ? GREEN_MOOD_VIEW : 
                                            color === "cyan" ? CYAN_MOOD_VIEW : 
                                            color === "blue" ? BLUE_MOOD_VIEW : 
                                            color === "purple" ? PURPLE_MOOD_VIEW : MOODS_VIEW

        const textStyle = action.color === "yellow" || action.color === "cyan" ? BLACK_MOOD_TEXT : WHITE_MOOD_TEXT
        ...
        switch (action.mood) {
          ...
          case "anxious":
            return {...state, anxious: {view: decodeStyleFromColor(action.color), text: textStyle}}
          default:
            return state
        }
  ...
  const onRelease = (colorValue, aspectX, aspectY) => {
    ...
    // angry
    if (isInRange(aspectX, state.angry.x, state.angry.x + state.angry.width) 
    ...
    // sad
    if (isInRange(aspectX, state.sad.x, state.sad.x + state.sad.width) 
    ...
    // happy
    if (isInRange(aspectX, state.happy.x, state.happy.x + state.happy.width) 
    ...
    // anxious
    if (isInRange(aspectX, state.anxious.x, state.anxious.x + state.anxious.width) 
  ...
  return <View style={ASPECT_CONTAINER}>
    <View>
